 ...
 * і замінює пробіли на підкреслення.
...
function customAssign(...objects) {
  ...
  const joined = Object.assign({}, ...objects);

  for (const key in joined) {
    if (typeof joined[key] === "number") {
      joined[key] = String(joined[key]);
    }
  }
  return joined;
}
...
/**
 * Функція `customObjectFromEntries` створює об'єкт з масиву записів, додатково
 * обробляючи значення властивостей.
...
function customObjectFromEntries(entries) {
  ...
  const resultObject = Object.fromEntries(arr);
  return resultObject;
}
 ...
 * властивостями другого і "заморожує" отриманий об'єкт, запобігаючи подальшим змінам.
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (Object.isFrozen(extend)) {
    return extend;
  } else {
    return "Error: the object was not frozen";
  }
}
...
/**
 * Функція keysAndValues приймає об'єкт, виводить в консоль його
 * ключі та значення, і повертає масив з масивами ключів та значень.
 * Використовуються методи Object.keys(), Object.values() та
 *  Object.entries() для реалізації.
 * obj - вхідний об'єкт.
...
function filterObjectsByKey(arr, key) {
  ...
  const filtered = arr.filter((obj) => obj.hasOwnProperty(key));
